...
def get_realtime_data():
    try:
        ...
        current_price = soxl['Close'].iloc[-1]  # [버그 2 수정] 여기서 한 번만 조회
...
def execute_dual_tactical_trader():
    ...
    # [버그 2 수정] current_price를 get_realtime_data()에서 함께 반환
    current_vix, prev_close, current_open, current_price = get_realtime_data()
    if current_vix is None:
        return

    # [버그 5 수정] prev_close 0 방어
    ...
    if shares_short > 0 and avg_short > 0:
        ...
        take_profit_price = avg_short * (1 + TAKE_PROFIT_RATIO)  # 익절 지정가 계산
