...
def main():
    ...
    for ticker in TICKERS:
        ...
        # [경고 박멸] .iloc[0, 0] 방식으로 스칼라 값을 직접 지정하여 추출
        ...
        # 3. [한글화] 리포트 메시지 구성
        header_title = "🚀 [즉시 익절 실행 권고]" if exit_ready else f"📊 {ticker} 시그마 전술 리포트 V3.4"
        
        rebalance_msg = "🚨 **하락장 대응**: SOXL 비중 25% 축소 추천\n\n" if bear_signal else \
                        "✅ **추세 전환**: 비중 50:50 재조정 추천\n\n" if bull_recovery else ""

        if exit_ready:
            sell_guide = "🔥 **[긴급] 과열 신호 포착! 현 시점 전량 익절 후 시즌 리셋을 권장합니다.**"
        elif CURRENT_USED >= ANNUAL_QUOTA:
            sell_guide = "🏁 **[종료] 올해 할당량(12회) 완료. 자산의 80%를 SPYM으로 전환하십시오.**"
        else:
            sell_guide = "⏳ 현재 시즌 운용 중 (설정된 LOC 가격에 기계적 매수 대기)"
        ...
        msg = (
            ...
            f"💰 현재 가격: ${latest_price:.2f} (내 평단 대비 {return_val:+.2f}%)\n"
            f"📍 적용 시그마 변동성: {dynamic_sigma*100:.2f}%\n\n"
            f"📈 **엔벨로프(120일) 분석**\n"
            f"  - 상단 저항선(25%): ${env_res_25:.2f}\n"
            f"  - 하단 지지선(25%): ${env_sup_25:.2f} {'⚠️ 하단 돌파!' if env_touched else ''}\n\n"
            f"{rebalance_msg}"
            f"🎯 **오늘의 매수 타점 (배분 4:6)**\n"
            f"📍 1차 예약(-1σ): **${p1:.2f}**\n"
            f"📍 2차 예약(-2σ): **${p2:.2f}**\n\n"
            f"📊 **시즌 집행 현황: {CURRENT_USED}/{ANNUAL_QUOTA}회**\n"
            f"🛑 **전술적 매도 가이드**\n"
            f"  {sell_guide}\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
    ...
    try:
        ...
        subprocess.run(["git", "commit", "-m", f"V3.4 Clean Report: {datetime.now().strftime('%Y-%m-%d %H:%M')}"], cwd=WORKING_DIR)
