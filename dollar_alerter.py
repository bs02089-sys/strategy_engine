...
판정 기준:
  - --monitor(장중)는 **실시간 가격**(yfinance, 15분 지연) 기준 — 나무증권
    달러 환전(평일 09:00~16:00 KST, 점검 23:50~00:10 제외) 가능 시간대에만
    신호 판정 (2026-08-18 확정: 야간 16:00~02:00 환전 불가).
...
def _bank_hours_open(now: datetime | None = None) -> bool:
    """나무증권 달러 환전 가능 시간 여부 — 신호 판정 게이트.

    나무증권 달러 환전은 **평일 09:00~16:00 KST** 에만 가능 (2026-08-18 조사 확정:
    '밤중에는 환전실행이 불가' — 블로그·고객센터 다수 확인. 야간 16:00~02:00는
    실제 환전 불가이므로 ponytail: 코드에서 제거).
    - 일일 점검 23:50~익일 00:10 제외 (NH투자증권 공지)
    - 주말(토·일) 휴무
    ...
    if wd >= 5:                     # 토(5)·일(6) — 휴무
        return False
    if m < 540:                     # 09:00 이전 — 휴무
        return False
    if m >= 960:                    # 16:00 이후 — 환전 불가
        return False
    # 일일 점검 23:50(1430)~00:10(10) 제외 — 16:00 이후이므로 위 조건으로 이미 차단됨
    return True
...
def compute_ticker(ticker: str, pos: dict, cfg: dict, live: bool = False) -> dict:
    ...
    gap_pct = float(cfg.get("IMMINENT_GAP_PCT", 0.2))
...
def detect_alerts(st: dict, pos: dict, cfg: dict) -> tuple[list[str], list[str], list[str]]:
    ...
    gap = float(cfg.get("IMMINENT_GAP_PCT", 0.2))
