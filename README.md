...
> 📱 **점검은 대시보드 카드로 바로 확인** — 카드 하단 `🛡 매도감시 점검` 줄이 계좌별로
> 🚀 지금 등록(목표 5%p 이내) / 🚨 매도 신호(도달) / ⏳ 등록 보류 + **등록 시작 가격**(목표 × 0.95)을
> 표시하고, 헤더에 건수를 요약합니다 (서버 렌더링 — `swing_personal.json` × `SWING_TARGET_PCT` 기준, 2026-09-12).
