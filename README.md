...
> 새 배포가 몇 분 안에 반영됩니다.
>
> **화면의 기준 표시 (2026-09-22)** — 헤더는 `업데이트 2026-09-22 09:20 KST`(**KST** 갱신 시각)와
> 가격 기준을 함께 보여줍니다: 장 마감 후에는 `종가 기준 09-21 (미국 ET)` 처럼 **그 종가의 세션
> 날짜**를 적고(생성 시각이 아님), 장중에는 `실시간(15분 지연) 기준 09-21 10:12 (미국 ET)` 로
> 표시합니다. 장중에는 가격뿐 아니라 **하락률·남은 %p·전일 종가 비교 기준도 라이브 세션** 으로
> 맞춰 화면 안 값이 서로 모순되지 않습니다 (전일 종가 = 라이브 세션의 직전 세션 종가, 날짜 병기).
> 종가 조회는 yfinance 일봉이 아직 확정되지 않았을 때(마지막 봉 Close=NaN) 정규장 분봉 종가로
> 폴백하므로, `종가 기준` 옆 날짜가 하루 낡은 세션으로 밀리지 않습니다.
> 알림 판정(구간 도달/임박/매도)은 표시와 무관하게 항상 **확정 종가 기준**입니다 — 장중 알림은
> Discord/OneSignal 푸시가 담당합니다.
