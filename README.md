...
- 각 티커별: 현재가, Sigma, LOC 목표가, 전고점 대비 하락률/회복률, 매수/매도 신호
  (전고점 = **원시 고가(High, 미조정)·전체 이력** 기준 — 스윙 알리미와 **공용 함수**
...
> **알림 = 스윙 알리미, 체결 = 나무증권 '해외주식 시세포착주문'** — 이중 장치가 서로를 백업합니다.
> 개인 포지션 기록은 `swing_personal.json` 한 곳뿐 (봇은 읽기만 — 절대 쓰지 않음).
> 상세 나무증권 등록 매뉴얼: [NAMU_SWING_SETUP.md](NAMU_SWING_SETUP.md)

**① 매수 체결 시 — LOTS에 계좌 기록** (`POSITIONS → TQQQ → LOTS`)

| 필드 | 내용 | 예시 |
|------|------|------|
| `ACCOUNT` | 계좌 번호 = 매수 구간 순서 (1번 -15% ~ 7번 -33%) | `2` |
| `BUY_PRICE` | 실제 매수가 | `73.49` |
| `SHARES` | 보유 수량 — **정수만** (소수는 버림) | `6` |

**② 나무증권 감시 등록 (시세포착주문)**

| 감시 | 등록 시점 | 조건 | 가격 | 기간 |
|------|----------|------|------|------|
| 매수감시 | 📡 매수 구간 **임박** 푸시 확인 시 | 하락 | 매수 구간 가격 (예: 2번 -18% $71.36) | 30일, 정규장만, 지정가+0틱 |
| 매도감시 | 🚀 매도 목표 **임박**(목표까지 5%p 이내) 시 | 상승 | 매수가 × 1.25 (예: $73.49 → **$91.86**) | 30일, 정규장만 |

> 🔍 목표율(`SWING_TARGET_PCT`)이나 전고점이 바뀐 뒤에는 나무증권 잔여 감시 주문을 점검해야 합니다
> (구 기준 감시는 체결되지 않거나 목표 전에 매도됨) — **`NAMU_SWING_SETUP.md` '감시 주문 점검 체크리스트'** 참고.

- 수량: 나무증권 '매수가능수량' 안내 기준, **$500 예산** 이내 (권장 6주, 체결가 변동 여유 1주)
- 30일 만료 후 미체결이면 **재등록** — 신호는 매수 🔻/📡, 매도 🚀 임박 (대시보드)
- ⚠️ 매도감시는 **목표 임박 시점에만** 등록 — 매수 직후 등록은 30일이 목표 도달(평균 8.3개월) 전에 만료됨

**③ 매도 완료 시 — LOTS 정리/재기록**

- 📈 매도 신호(또는 매도감시 자동 체결) → 수동 매도 확인 → 해당 계좌 `BUY_PRICE`/`SHARES` 비우기
- 전 계좌 매도 완료 시 봇이 **자동 리셋(🔄)** — 수동 `--reset` 불필요

**④ 신호 종류 (푸시 = 전체 구독자 = 내 기기)**

| 신호 | 의미 | 대응 |
|------|------|------|
| 📡 매수 구간 임박 | 다음 구간(ATH 대비 -N%)까지 5%p 이내 | 나무증권 매수감시 등록 (30일) |
| 🔻 매수 구간 도달 | ATH 대비 -N% 도달 | 체결 확인 → LOTS 기록 |
| 🚀 매도 목표 임박 | 매수가 대비 +25%까지 5%p 이내 | 나무증권 매도감시 등록 (30일) |
| 📈 매도 신호 | 매수가 대비 +25% 도달 | 수동 매도 → LOTS 정리 |
| 🔄 사이클 완료 | 전 계좌 매도 → 자동 리셋 | 기록 불필요 (다음 사이클 시작) |
...
LOC_DCA(시그마)보다 우월한지 비교합니다 (2026-08-17). **스윙의 매도 목표(`SWING_TARGET_PCT`)는
제외하고 무매도 축적**으로 동일 예산($50,000)을 굴린 결과를 봅니다.
...
- 가격 기준(2026-09-12 정렬): **원시 종가/고가(미조정)를 두 전략이 공통 사용** — 스윙 ATH 는 엔진
  공용 `get_all_time_high`(High 기준)와 동일. 이전에는 배당 조정 종가를 써서 스윙 ATH 가 실전보다
  약 1.2% 낮았고, 그 차이가 5년 결론(스윙 우위)을 뒤집을 만큼 컸다 (아래 수치 참고)
- ⚠️ 결과는 윈도우 시작 시점에 민감 — 10년(2016-08~)은 **LOC 우위**(+2536% vs 스윙 +1081%),
  5년(2021-08~, 고점 부근)은 **거의 동률**(LOC +94.1% vs 스윙 +91.3%, B&H +94.6%) — 두 윈도우
  모두 B&H 와의 차이보다 두 전략 간 차이가 작다 (LOC의 시그마 트리거는 어떤 변동성에서도 발동해
  저점에서 즉시 매수, 스윙은 ATH 하락 구간 도달까지 대기 — 이 대기가 강세장에서 수익률을 깎는다)
- ⚠️ `--rolling` 강건성 검증 (2026-09-12 재측정): 14개 고정 5년 윈도우에서 스윙 9승/LOC 5승이지만
  **평균 수익률 동률**(스윙 +394.2% vs LOC +389.1%, B&H +431.7%)·MDD 동일(-75.6/-75.8%) —
  인접 윈도우(2개월 차이)에서도 승자가 뒤집혀 **5년 우위는 윈도우 운이지 전략 우월이 아님**
  (구 배당조정 기준 2026-08-17 측정: 스윙 11승/LOC 3승 · 스윙 +397.0% vs LOC +395.0%)
- ⚠️ `--swing-zones 20`/`--sweep-zones` (2026-09-12 재측정): 구간을 20차(-72%)까지 깊게 나누면
  **평균현금비율이 3.1%→8.4%로 상승(자금 유휴)하고 평균 수익률도 +394%→+317%로 하락** —
  평균 수익률 최적은 **7구간(-15%~-33%, 현재 스윙 설정 그대로)**, 10구간(-42%)까지가 실용 한계
  (⚠️ 최근 5년 크래시 윈도우만 보면 반대로 20구간 +147.8% 가 최고 — 단 이 모델은 **매도 없는
   무매도 축적**이라 깊은 래더가 유리하게 나온다. 실전 모델(매도 후 재매수)로 돌리면
   3~5% 스텝·7계좌가 우위 — 아래 '스윙 래더 검증' 항목 참고)
- ⚠️ `--sweep-splits` (2026-09-12 재측정): '1년 매수 + 4년 홀딩'에서 분할 수가 적을수록 평균 수익률이
  높다 — 1분할(≈일시 매수≈B&H) +404% > **현행 5분할 +389%** > 10분할 +383% > 20분할 +298% >
  52분할 +125% (1년차 트리거 부족으로 유휴). MDD는 1~10분할 동일(-75.8%) — 분할 수는 MDD와
  무관하고 '강세장에서 늦은 투입'이 수익률 차이의 원인. 최근 5년(크래시 윈도우)만 보면 반대로
  52분할이 최고(+97.8%) — 또 한 번 윈도우 의존성. → **2026-08-17 실전 20→5분할 채택**
- ⚠️ 위 백테스트 수치는 **2026-09-12 재측정**: ① 2026-08-17 LOC 판정 마감가(종가) 기준 수정,
  ② 2026-09-12 가격 계열을 실전과 동일한 **원시 종가/고가(미조정)** 로 통일 (구 배당 조정 기준
  수치는 무효 — [스윙 백테스트 기준 정렬과 동일 이유])

### 스윙 래더 검증 — 7계좌 × 3% 스텝 (`swing_split_backtest.py`)

세븐 스플릿(계좌 1~7 × $500, ATH -15%~-33% 3% 래더, 매도 +25%(`SWING_TARGET_PCT`) 후 재매수)이
크래시 국면에서도 적절한지 재검증했습니다 (2026-09-12, 원시 High/종가 기준 정렬 + 목표 +25% 기준 ·
크래시 직전 고점에서 시작하는 5년 윈도우 5개: 2018-10 / 2020-02 코로나 / 2021-11 / 2021-12 / 2022-08):

- **스텝(래더 깊이)**: 3%(-33% 종료) 평균 +136%·최악 +99% / 2% +129% / 4% +126% / 5% +122% / 10%(-75%) +101%
  → **3%가 평균 1위, 2~5% 차이는 ±7%p 이내** (현행 3% 유지 타당)
- **계좌 수**: 7계좌가 평균 +136%·최악 +99%로 **1위** (3계좌 +121%·최악 +63% / 5계좌 +130%·+84%
  / 10계좌 +123%·+97%) → **세븐 스플릿 구조 유지 타당**
- **첫 구간**: 현행 -15% 가 무난 — -18% 와 평균 동률(+136%)이고 최악만 -18%가 +111%로 근소 우세
  (2018·2020 크래시에선 -10~-15% 우세) → 윈도우 의존이라 변경 근거로는 약함
- **매도 목표**: 10년 Sharpe 는 +25% 0.85(최적) > +40% 0.79, 평균보유 109일 vs 194일 (회전 우위) —
  다만 크래시 5윈도우 수익률은 +40% 가 높다(평균 +143% vs +136%) — **회전율과 수익률의 맞교환**
  (2026-08-31 +25% 전환은 Sharpe·회전 근거)
- ⚠️ **모델 차이 주의**: `loc_vs_swing_backtest.py --sweep-zones` 는 *무매도 축적* 모델이라 깊은
  래더(20구간)가 유리하게 나오지만, 실전 모델(매도 후 재매수)에서는 7계좌·3~5% 가 우위 —
  래더 형상 판단은 이 항목(실전 모델)을 기준으로 할 것
- ⚠️ 실행: `python3 swing_split_backtest.py --steps 1,2,3,4,5,6,8,10 --since 2021-09-13 --no-index`
...
- **매수**: 역대 최고가(ATH) 대비 MDD 5% 단위 구간(-5% ~ -95%)에 현재가가 도달하면
  해당 구간이 '매수' 상태가 됩니다.
- **매도**: 실제 매수가(`BUY_PRICE`) 대비 스윙 목표 수익률(`SWING_TARGET_PCT`, 현재 +25%) 도달 시
  매도 알람 (예: 매수가 $100 → 목표 $125). 앱 대시보드의 기본 선택 수익률도 이 설정값을 읽습니다.
...
📱 **나무증권 시세포착주문 감시 등록 가이드**: [NAMU_SWING_SETUP.md](NAMU_SWING_SETUP.md) —
...
- **전 계좌 매도 완료 자동 리셋 (2026-08-11)**: `LOTS` 의 모든 계좌가 매도 목표(+25%)에 도달하면
...
- 기준가는 기본 `ATH`(역대 최고가)이며 **원시 고가(High, 미조정) 기준**으로 계산해
  Google Finance `=GOOGLEFINANCE(TICKER, "high52")`와 일치합니다. (실거래 장중 전고가 기준 — 2026-08-20 변경)
...
`SWING_TARGET_PCT` 설정값(현재 +25% — 2026-08-31 +40%→+25% 전환)이 단일 소스입니다.
...
- **매도 목표 임박 푸시 (2026-08-15)**: 개인 포지션의 매도 목표 임박(🚀, 목표까지 `IMMINENT_GAP_PCT`
  ...
  (NAMU_SWING_SETUP.md ②)의 신호**로 활용 (계좌별 사이클당 1회 — 매도 신호 푸시와 동일 패턴).
...
#### 예시 (TQQQ 현재가 $74.47, SWING_TARGET_PCT 25%)

| 매수 예정가 (서버 LOTS) | 매도 예정가 (= 매수 × 1.25) | 현재가 기준 상태 |
|---|---|---|
| (미입력 — null) | — | — |
| $60.00 | $75.00 | ⏳ 대기 |
| $50.00 | $62.50 | 🚨 매도 (이미 도달) |
...
| [NAMU_SWING_SETUP.md](NAMU_SWING_SETUP.md) | 나무증권 시세포착주문 감시 등록 가이드 (스윙 알리미) |
