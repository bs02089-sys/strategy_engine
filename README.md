...
### 매도 알람 — 수익률 기준 (앱 대시보드 + 서버)

대시보드의 **매도 상태 칩**(🚨 매도 / 🚀 임박 / ⏳ 대기)과 상단 **"🚨 매도 알람 N"** 카운트는
**각 사용자가 입력한 매수가와 선택한 수익률**(5/10/15/20%, 브라우저 localStorage) 기준으로
자동 판정됩니다. 서버 설정 없이도 사용자별로 동작합니다.

- **매수가 입력**: 앱에서 사용자가 직접 입력합니다. 비워두면 **현재가 (지금 매수 시 기준)** 로
  동작합니다.
- **매도가 = 매수가 × (1 + 수익률/100)** — 실시간 자동 계산.
- **판정**: 현재가 ≥ 매도가 → **🚨 매도** (빨강) / 목표까지 `IMMINENT_GAP_PCT`(기본 5%p) 이내 →
  **🚀 임박** (호박) / 그 외 → **⏳ 대기**
...
- **저장**: 매수가(`swing_buy_{TICKER}`)·수익률(`swing_sell_{TICKER}`)은 브라우저
  localStorage에 기기별로 독립 저장됩니다.

#### 예시 (TQQQ 현재가 $74.47)

| 내 매수가 | 수익률 10% 매도가 | 현재가 기준 상태 |
|---|---|---|
