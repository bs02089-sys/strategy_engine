...
| 매도감시 | 🚀 매도 목표 **임박**(목표까지 5%p 이내) 시 | 상승 | 매수가 × 1.4 (예: $73.49 → **$102.89**) | 30일, 정규장만 |
...
| 🚀 매도 목표 임박 | 매수가 대비 +40%까지 5%p 이내 | 나무증권 매도감시 등록 (30일) |
| 📈 매도 신호 | 매수가 대비 +40% 도달 | 수동 매도 → LOTS 정리 |
...
- ⚠️ `--swing-zones 20`/`--sweep-zones` (2026-09-12 재측정): 구간을 20차(-72%)까지 깊게 나누면
  **평균현금비율이 3.1%→8.4%로 상승(자금 유휴)하고 평균 수익률도 +394%→+317%로 하락** —
  ...
  (⚠️ 최근 5년 크래시 윈도우만 보면 반대로 20구간 +147.8% 가 최고 — 단 이 모델은 **매도 없는
   무매도 축적**이라 깊은 래더가 유리하게 나온다. 실전 모델(+40% 매도 후 재매수)로 돌리면
...
세븐 스플릿(계좌 1~7 × $500, ATH -15%~-33% 3% 래더, 매도 +40% 후 재매수)이 크래시 국면에서도
적절한지 재검증했습니다 (2026-09-12, 원시 High/종가 기준 정렬 후 · 크래시 직전 고점에서 시작하는
5년 윈도우 5개: 2018-10 / 2020-02 코로나 / 2021-11 / 2021-12 / 2022-08):

- **스텝(래더 깊이)**: 3%(-33% 종료) 평균 +143%·최악 +110% / 4% +140% / 5% +132% / 10%(-75%) +115%
  → **3~5% 가 상위권이고 차이는 ±5%p 이내** (현행 3% 유지 타당)
- **계좌 수**: 7계좌가 평균 +143%·최악 +110%로 **1위** (3계좌 +133%·최악 +82% / 10계좌 +135%·+103%)
  → 섬븐 스플릿 구조 유지 타당
- **첫 구간**: 현행 -15% 가 중간값으로 무난 — -18% 시작이 평균 +147%로 근소 우위지만 2018·2020
  크래시에선 -10~-15% 가 우세해 윈도우 의존 (변경 근거로는 약함)
- ⚠️ **모델 차이 주의**: `loc_vs_swing_backtest.py --sweep-zones` 는 *무매도 축적* 모델이라 깊은
  래더(20구간)가 유리하게 나오지만, 실전 모델(매도 +40% 후 재매수)에서는 7계좌·3~5% 가 우위 —
