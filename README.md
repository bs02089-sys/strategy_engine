...
**각 사용자가 입력한 매수 예정가와 선택한 수익률**(5/10/15/20%, 브라우저 localStorage) 기준으로
자동 판정됩니다. 서버 설정 없이도 사용자별로 동작합니다.

- **매수 예정가 입력**: 앱에서 사용자가 직접 입력합니다. 비워두면 **현재가 (지금 매수 시 기준)** 로
  동작합니다.
- **매도 예정가 = 매수 예정가 × (1 + 수익률/100)** — 실시간 자동 계산.
...
- **저장**: 매수 예정가(`swing_buy_{TICKER}`)·수익률(`swing_sell_{TICKER}`)은 브라우저
  localStorage에 기기별로 독립 저장됩니다.

#### 예시 (TQQQ 현재가 $74.47)

| 내 매수 예정가 | 수익률 10% 매도 예정가 | 현재가 기준 상태 |
|---|---|---|
