...
- **전역 OneSignal 푸시는 제거됨 (2026-08-10)**: `--monitor`가 신호 요약을 구독자 전체에게
  ...
  기준 사용자별 푸시로만 발송됩니다.
- **매수 구간 푸시 (2026-08-11)**: 매수 구간 도달(🔻)/임박(📡)은 Discord뿐 아니라 **앱을 연
  구독자에게도 사용자별 태그 푸시**로 발송됩니다 — 디스코드 미가입 지인도 스마트폰으로 수신 가능.
  앱을 열면 `swing_zone_{TICKER}` 태그가 자동 등록되고(별도 설정 불필요), 서버가 태그 필터로
  발송합니다. ATH(공개 정보) 기준이라 전역 푸시 금지 규칙과 충돌하지 않습니다.
...
- **계좌별 매수 예정가 입력**: 세븐 스플릿에 맞춰 **계좌 1~7행**을 입력합니다 (각 $500).
  **매수한 계좌만 입력**하며, 미입력 계좌는 매수/매도 예정가 모두 비워 둡니다 (현재가 자동
  표시 없음 — 2026-08-12 변경). 입력한 계좌만 계산/저장/푸시에 반영되고, 미입력 계좌는
  무시됩니다 (오발송 방지).
- **매도 예정가 = 매수 예정가 × (1 + 예상 수익률/100)** — 계좌별 실시간 자동 계산 (매수 예정가 입력 시).
...
- **매수 구간 푸시 (2026-08-11)**: 앱을 열면 `swing_zone_{TICKER}` 태그가 자동 등록되어, 매수
  구간 도달(🔻)/임박(📡) 발생 시 해당 티커 구독자 전원에게 푸시가 발송됩니다 (별도 설정 불필요 —
  앱을 켠 것 자체가 수신 동의). 개인 정보가 없는 공개 신호라 전역 발송이 아닌 태그 필터로
  보내며, 중복 방지는 서버 `ZONE_ALERTS` 상태가 담당하고 발송 실패 시 다음 폴링에서 재시도합니다
  (하루 지난 대기분은 폐기 — 스테일 발송 방지).
...
- **기기 간 동기화 (2026-08-12)**: 헤더의 **🔄 동기화 코드**에 **두 기기에 같은 코드**를 입력하면
  OneSignal 외부 ID로 기기들이 한 사용자로 연결되어 예상 수익률(`swing_pct_{TICKER}`)·계좌별 매수
  예정가(`swing_buy_{TICKER}_{ACCOUNT}`) 태그가 공유됩니다. 페이지 로드 시 태그 값을 우선 채택하므로
  한 기기에서 바꾸면 다른 연결 기기의 매도 예정가도 자동으로 일치합니다 (마지막 변경 기기가 기준).
  코드를 비우면 연결이 해제되어 기기별 저장으로 돌아갑니다. 코드 입력칸은 🔒 가려진 상태로
  표시되며(눈 아이콘으로 잠시 확인), 코드는 화면·기기 localStorage·OneSignal 사용자 레코드에만
  존재합니다 (커밋/공용 알림 노출 없음). ⚠️ 연결 시 매수 예정가가 OneSignal 사용자 레코드에
  저장되므로 개인 정보 취급에 유의하세요 (공용 알림 노출 경로는 아닙니다).
  ⚠️ 계좌를 비우는(삭제) 동작은 태그가 제거되어도 다른 기기에는 전파되지 않으므로, 연결된
  기기에서도 직접 비워주세요 (입력/변경 값만 동기화).
