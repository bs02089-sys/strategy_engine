...
`setup_cronjob_org.py --swing`이 4h봉 마감 직후(월~금 **18:45/22:45 UTC**)에
`repository_dispatch(swing-bot)`를 발사하는 크론잡을 만듭니다. GitHub Actions 자체
스케줄 크론은 best-effort라 지연/비활성화될 수 있어, 정확한 시각 알림으로 우회합니다.
...
- 여름(DST)엔 4h봉 마감이 17:30/21:30 UTC, 겨울엔 18:30/22:30 UTC — 18:45/22:45 UTC
  실행은 두 계절 모두 '마감 직후'가 됩니다.
- `swing_bot.yml`은 `repository_dispatch(swing-bot)` 트리거를 이미 지원합니다 (코드 푸시 후
  사용). 기존 `schedule` 크론은 보조 수단으로 그대로 유지됩니다 — schedule(18:00/22:00 UTC)과
  dispatch(18:45/22:45 UTC)가 같은 완성 봉을 병행 분석할 수 있지만, 상태 머신이 중복
  BUY/SELL 알림을 차단합니다.
