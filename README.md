...
- ⚠️ `--rolling` 강건성 검증 (2026-08-17): 14개 고정 5년 윈도우에서 스윙 11승/LOC 3승이지만
  **평균 수익률 동률**(스윙 +397% vs LOC +395%, B&H +438%)·MDD 동일(-75.6/-75.7%) —
  인접 윈도우(2개월 차이)에서도 승자가 뒤집혀 **5년 우위는 윈도우 운이지 전략 우월이 아님**
- ⚠️ `--swing-zones 20`/`--sweep-zones` (2026-08-17): 구간을 20차(-72%)까지 깊게 나누면
  **평균현금비율이 2.8%→8.9%로 상승(자금 유휴)하고 평균 수익률도 +397%→+315%로 하락** —
  평균 수익률 최적은 **7구간(-15%~-33%, 현재 스윙 설정 그대로)**, 10구간(-42%)까지가 실용 한계
- ⚠️ `--sweep-splits` (2026-08-17): '1년 매수 + 4년 홀딩'에서 분할 수가 적을수록 평균 수익률이
  높다 — 1분할(≈일시 매수≈B&H) +410% > **현행 5분할 +395%** > 10분할 +387% > 20분할 +299% >
  52분할 +125% (1년차 트리거 부족으로 유휴). MDD는 1~10분할 동일(-75.7%) — 분할 수는 MDD와
  무관하고 '강세장에서 늦은 투입'이 수익률 차이의 원인. 최근 5년(크래시 윈도우)만 보면 반대로
  52분할이 최고(+107%) — 또 한 번 윈도우 의존성. → **2026-08-17 실전 20→5분할 채택**
- ⚠️ 위 백테스트 수치는 2026-08-17 **LOC 판정을 마감가(종가) 기준으로 수정 + 승수 1.43 전환 후
  재측정**한 값 (구 저가 기준 수치는 무효)
