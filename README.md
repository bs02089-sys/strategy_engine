...
`SWING_TARGET_PCT` 설정값(현재 20%)이 단일 소스입니다.
...
- **동기화 코드 (2026-08-12 이후)**: 헤더의 **🔄 동기화 코드**는 더 이상 값 동기화가 아니라
  OneSignal **외부 ID 사용자 병합** 용도입니다 (모닝 리마인더 푸시 타깃팅에 사용). 같은 코드를
  두 기기에 입력하면 OneSignal 사용자가 병합되지만, **매수/매도 예정가의 기기 간 일치는
  `swing_personal.json` 서버 렌더링이 담당**합니다. 코드 입력칸은 🔒 가려진 상태로 표시되며
  (눈 아이콘으로 잠시 확인), 코드는 화면·기기 localStorage·OneSignal 사용자 레코드에만
  존재합니다 (커밋/공용 알림 노출 없음).

#### 예시 (TQQQ 현재가 $74.47, SWING_TARGET_PCT 20%)

| 매수 예정가 (서버 LOTS) | 매도 예정가 (= 매수 × 1.2) | 현재가 기준 상태 |
|---|---|---|
| (미입력 — null) | — | — |
| $60.00 | $72.00 | ⏳ 대기 |
| $55.00 | $66.00 | 🚨 매도 (이미 도달) |
