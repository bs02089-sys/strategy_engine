...
영상 로직 → 코드 구현 매핑:
  ...
  6. 무인(수면) 운용 — 진입 알림의 ①~④ 체크리스트(진입 지정가/SL/TP/MOC)를 브로커 OCO/브래킷
     그룹으로 걸면 TP/손절/마감 청산을 브로커가 자동 처리 → 이후 TP/손절/MOC 알림은
     아침에 확인하는 기록. MOC 미지원 브로커는 15:40 당일 마감 알림으로 직접 청산.
...
def build_message(signal):
  ...
  return (
      ...
      f"📋 **브로커 주문 체크리스트** — ①~④를 한 OCO/브래킷 그룹으로 주문\n"
      f"  ① 진입 지정가: `{signal['entry']:.2f}`\n"
      f"  ② 손절 Stop: `{signal['stop_loss']:.2f}` 🛑\n"
      f"  ③ 익절 Limit: `{signal['take_profit']:.2f}` 🎯\n"
      f"  ④ MOC 매도 **ON** — 미체결 시 16:00 마감 경매 자동 청산\n\n"
      f"👉 OCO로 묶으면 한쪽 체결 시 나머지가 자동 취소됩니다. "
      f"주문 후 주무시면 TP/손절/MOC 청산은 전부 브로커가 처리합니다.\n"
      f"⏰ 이후 TP/손절/MOC 알림은 아침에 확인할 기록입니다. "
      f"MOC를 못 걸었다면 15:40 당일 마감 알림으로 직접 청산하세요."
  )
