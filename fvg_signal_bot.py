...
영상 로직 → 코드 구현 매핑:
  ...
  6. 무인(수면) 운용 — 나무증권(NH) 기준: ① 진입 지정가 + ②③ 시세포착주문(서버 24시간
     감시, 손절/익절 자동 매도) + ④ MOC(마감 경매 자동 청산)를 걸면 자는 동안 청산이
     자동 처리된다 → 이후 TP/손절/MOC 알림은 아침에 확인하는 기록. 상세: FVG_NAMYU_SETUP.md
...
def build_message(signal):
  ...
  return (
      ...
      f"  ② 시세포착주문 매도 — 손절(구조 저점 아래): `{signal['stop_loss']:.2f}` 🛑\n"
      f"  ③ 시세포착주문 매도 — 익절(선택): `{signal['take_profit']:.2f}` 🎯\n"
      f"  ④ MOC 매도 **ON** — 미체결 시 16:00 마감 경매 자동 청산\n\n"
      f"👉 ②③은 서버가 24시간 감시하는 시세포착주문입니다 — 자는 동안에도 자동 매도됩니다 "
      f"(해외주식 > 시세포착주문, 신규편입 조건으로 미리 등록 가능).\n"
