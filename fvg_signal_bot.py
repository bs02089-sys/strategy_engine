...
영상 로직 → 코드 구현 매핑:
  ...
  6. 무인(수면) 운용 — 나무증권(NH) 기준: ① 진입 지정가 + ② 시세포착주문 매도 1건
     (손절+익절 동시 입력, 서버 24시간 감시 자동 매도) + ③ MOC(마감 경매 자동 청산)를
     걸면 자는 동안 청산이 자동 처리된다 → 이후 TP/손절/MOC 알림은 아침에 확인하는 기록.
...
def build_message(signal):
  ...
  return (
      ...
      f"  ① 진입 지정가 매수: `{signal['entry']:.2f}`\n"
      f"  ② 시세포착주문 매도 1건 — 손절+익절 동시 입력:\n"
      f"     · 손절(구조 저점 아래): `{signal['stop_loss']:.2f}` 🛑\n"
      f"     · 익절(선택): `{signal['take_profit']:.2f}` 🎯\n"
      f"  ③ MOC 매도 **ON** — 미체결 시 16:00 마감 경매 자동 청산\n\n"
      f"👉 ②는 서버가 24시간 감시 — 둘 중 먼저 도달한 쪽으로 자동 매도 후 감시 종료됩니다 "
      f"(해외주식 > 시세포착주문, 신규편입으로 매수 전 미리 등록 가능).\n"
