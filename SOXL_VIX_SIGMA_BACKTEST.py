...
def verify_long_term_hold_logic():
    print("📡 [SOXL_VIX_SIGMA_BACKTEST_LOC.py] LOC(종가) 기준 정밀 백테스트")
    print("🎯 확실한 하락 마감일 종가 저격망 및 연간 쿼터 최적화 관제탑\n")
    ...
    def evaluate_parameters(normal, fear, extreme):
        ...
        # 🎯 핵심 변경점: 장중 저점(Low)이 아니라 '종가(Close)'가 타깃가 이하일 때만 매수 격발! (LOC 구현)
        triggered = df['Close'].values <= target_prices
        
        # LOC 매수는 종가 체결이 원칙이므로 체결가를 Close로 고정합니다.
    ...
    print("[현재 config.json 설정값 기반 LOC 성과]")
    print(f"   평시: {CURRENT_NORMAL:.2f} | 공포: {CURRENT_FEAR:.2f} | 극단: {CURRENT_EXTREME:.2f}")
    print(f"   💥 총 LOC 매수 횟수 : {int(cur_triggered.sum())}회 (4년 총합)")
    print(f"   📊 연평균 매수 빈도 : {int(cur_triggered.sum() / total_years)}회 ➔ 🔥 확실히 줄어든 쿼터 후보!")
    ...
    # ==================== 5. 초고속 최적화 루프 (LOC 맞춤형) ====================
    print("🔍 LOC 매수 기준 안전 범위 내 최적 배수 탐색 중...")
    ...
    for normal, fear, extreme in product(normal_range, fear_range, extreme_range):
        ...
        # LOC는 횟수가 적으므로, 4년간 최소 20번 이상 사지는 현실적인 조합 탐색
        if trade_count < 20:
            continue
    ...
    if best_params:
        ...
        print("="*65)
        print("💡 config.json에 반영할 스펙 (LOC 매수 최적화):")
        ...
        if answer == "y":
            try:
                ...
                print("✅ config.json에 LOC 기준 최적 배수 및 연간 쿼터가 연동 저장되었습니다.")
            except Exception as e:
                print(f"❌ config.json 저장 실패: {e}")
    else:
        print("❌ LOC 기준 최적 배수 조합을 찾지 못했습니다.")
