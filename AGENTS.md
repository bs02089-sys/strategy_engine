...
- **스윙 알리미**: `swing_alerter.py` (2026-08-08 신규) — 유튜브 'TQQQ 스윙 투자 전략' 구글 스프레드시트
  ...
  **전체 구독자(Subscribed Users = 내 기기) 대상**으로 발송(2026-08-12 단독 사용 전환 — 매도 신호 푸시는
  ...
  동기화가 아니다.
  🛡 **대시보드 매도감시 점검 표시 (2026-09-12)**: 카드 하단에 계좌별 '등록 필요 시점'을 함께 그린다 —
  같은 서버 렌더링 값(LOTS `sell_gap_pct`/`sell_ready`)으로 🚀 지금 등록(목표까지 IMMINENT_GAP_PCT
  이내) · 🚨 매도 신호(도달) · ⏳ 등록 보류를 구분하고 헤더에 '지금 등록 N · 매도 신호 N · 등록 보류 N'을
  요약한다. 등록 보류 줄에는 **등록 시작 가격**(목표 × (1 − IMMINENT_GAP_PCT/100))을 함께 표시해
  '몇 달러에 닿으면 등록하는지'를 알려준다. 목적: 나무증권 감시 점검(NAMU_SWING_SETUP.md 체크리스트)의
  실행 트리거를 화면에서 바로 보이게 하는 것 — 매수 직후 등록은 30일 만료가 먼저 오므로 '지금 등록'은 임박부터다.
  서버 HTML/CSS 전용(JS·localStorage 무관)이라 세션용 수익률 버튼과 무관하게 항상 같은 상태를 보여준다.
